...
for k = 1:length(jpegFiles_ban)
  baseFileName = jpegFiles_ban(k).name;
  fullFileName = fullfile(banana_folder, baseFileName);
  fprintf(1, 'Now reading %s\n', fullFileName);
  imageArray = imread(fullFileName);
...
[app_labels app_bags] = extractinstances2(app_images,40,1);
[banana_labels banana_bags] = extractinstances2(banana_images,40,2);
disp("end");
save('app_extract.mat','app_bags');
save('banana_extract.mat','banana_bags');
%%

A = load('app_extract.mat');
A = A.app_bags;
...
%%
% app_labels = {};
% for k = 1:length(app_bags)
%    app_labels = [app_labels,1];
% end
% banana_labels = {};
% for k = 1:length(banana_bags)
%    banana_labels = [banana_labels,2];
% end
